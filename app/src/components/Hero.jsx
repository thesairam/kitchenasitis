...
export default function Hero() {
  ...
  return (
    <section
      id="top"
      className="grain relative min-h-[100dvh] flex items-center pt-28 pb-16 px-5 md:px-10 bg-forest text-cream"
    >
      <div className="max-w-7xl mx-auto w-full grid md:grid-cols-12 gap-8 md:gap-10">
        <div className="md:col-span-8">
          ...
          <motion.p
            ...
            className="mt-6 md:mt-8 text-base md:text-xl text-cream/70 max-w-xl leading-relaxed"
          >
          ...
          >
            <a href="#order" className="bg-spice text-cream px-6 py-3 rounded-full text-sm hover:bg-cream hover:text-ink transition-colors">
              Order on Kookxtra
            </a>
            <a href="#story" className="border border-cream/30 text-cream px-6 py-3 rounded-full text-sm hover:bg-cream/10 transition-colors">
              Read the story
            </a>
          </motion.div>
        </div>
        ...
        <div className="md:col-span-4 flex md:justify-end md:items-end mt-4 md:mt-0">
          <div className="text-left md:text-right">
            <div className="text-xs tracking-widest uppercase text-cream/40 mb-2">This week</div>
            <div className="font-display text-3xl md:text-5xl relative h-12 md:h-16">
              {SPICES.map((s, i) => (
                <motion.span
                  key={s}
                  className="absolute left-0 md:right-0 md:left-auto text-cream/80"
      ...
      <div className="absolute right-[-8vw] top-1/2 -translate-y-1/2 w-[55vw] max-w-[580px] aspect-square rounded-full border border-cream/5 pointer-events-none" />
      <div className="absolute right-[-3vw] top-1/2 -translate-y-1/2 w-[40vw] max-w-[420px] aspect-square rounded-full border border-spice/15 pointer-events-none" />
    </section>
  )
}
